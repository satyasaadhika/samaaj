...
const StoryViewer = ({viewStory, setViewStory}) => {
    ...
    useEffect(() => {
      ...
      if(viewStory && viewStory.media_type !== 'video') {
        ...
        // Timer to close the story after duration(10sec)
        timer = setTimeout(() => {
          setViewStory(null);
        }, duration);
      }
    ...
    const renderContent = () => {
      switch(viewStory.media_type) {
        case 'image':
          return(
            <img src={viewStory.media_url} alt='story media' className='max-w-full max-h-full object-contain'/>
          );
        case 'video':
          return(
  ...
  return (
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color : '#000000'}}>
      ...
      <div className='absolute top-4 left-4 flex items-center space-x-3 p-2 px-4 sm:p-4 sm:px-8 backdrop-blur-2xl rounded bg-black/50'>
        <img src={viewStory.user?.profile_picture} alt='profile picture' className='ize-7 sm:size-8 rounded-full object-cover border border-white'/>
